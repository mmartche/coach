...
//        return '<img class="avatar" src="' . $default .'" width="' . $size . '" height="' . $size . '" alt="' . $alt . '" />';
    ...
    public static function isEnquiryProduct($obj, $product) {
        if (!is_array($product)) {
            return (bool)$obj->journal2->settings->get('enquiry_products.' . $product, false);
        }
        ...
        if ($obj->journal2->settings->get('enquiry_products') === 'outofstock' && isset($product['labels']) && isset($product['labels']['outofstock'])) {
            return true;
        }

        return (bool)$obj->journal2->settings->get('enquiry_products.' . $product['product_id'], false);
    }
