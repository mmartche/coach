...
$sql_list_email = "SELECT * FROM coach_ativocoach_email WHERE exact_return != null";
...
foreach ($result_list_email as $result_list => $key) {
    // var_dump($key["sender_to"], $key["message"], "CadastroconcluidoCoach", $key["subject"]);
    $varvar = exactEmail($key["sender_to"], $key["message"], "CadastroconcluidoCoach", $key["subject"]);
